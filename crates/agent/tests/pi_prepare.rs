...
fn json_happy_path_seeds_renders_guards_and_inits_wiki() {
    ...
    assert!(
        steps[0]["detail"].is_string(),
        "every step carries a detail"
    );
    assert_eq!(
        v["agentDir"].as_str(),
...
fn exit_9_when_the_cwd_is_not_a_project_root() {
    ...
    // A failed root guard must not create a project wiki.
    assert_eq!(
        steps
            .iter()
            .find(|s| s["name"] == "wiki")
            .expect("wiki step")["status"]
            .as_str(),
        Some("SKIPPED")
    );
    assert_eq!(
        steps
            .iter()
            .find(|s| s["name"] == "wiki")
            .expect("wiki step")["detail"]
            .as_str(),
        Some("wiki init skipped (root guard failed)")
    );
    assert!(
        !project.join("wiki").exists(),
        "a failed root guard creates no wiki directory"
    );
}

#[test]
fn reseed_preserves_edited_files_refreshes_unedited_files_and_restores_missing_files() {
    let tmpl = template("reseed-tmpl");
    let agent_dir = fresh_dir("reseed-agent");
    let project = fresh_dir("reseed-project");
    ...
    write(&agent_dir.join("prompts/brief.md"), "user prompt edits\n");
    write(&tmpl.join("prompts/brief.md"), "template prompt v2\n");
    write(&tmpl.join("settings.json"), "{\"version\": 2}\n");
    std::fs::remove_file(agent_dir.join("skills/dev/SKILL.md")).expect("remove managed file");
    ...
    let report_json = json_out(&second);
    let report = report_json["steps"]
        .as_array()
        .expect("steps")
        .iter()
        .find(|step| step["name"] == "seed")
        .expect("seed step");
    assert_eq!(report["status"].as_str(), Some("OK"));
    let detail = report["detail"].as_str().expect("seed detail");
    assert!(
        detail.contains("kept_edited [prompts/brief.md]"),
        "detail: {detail}"
    );
    assert_eq!(
        std::fs::read_to_string(agent_dir.join("prompts/brief.md")).expect("edited prompt"),
        "user prompt edits\n",
        "user edits survive a stamp bump"
    );
    assert_eq!(
        std::fs::read_to_string(agent_dir.join("settings.json")).expect("refreshed settings"),
        "{\"version\": 2}\n",
        "an unedited managed file is refreshed"
    );
    assert_eq!(
        std::fs::read_to_string(agent_dir.join("skills/dev/SKILL.md")).expect("restored skill"),
        "# dev\n",
        "a missing managed file is restored"
    );
    let manifest =
        std::fs::read_to_string(agent_dir.join(".terax-seed-manifest")).expect("manifest");
    assert!(
        ...
        "manifest has a SHA-256 entry for every managed file: {manifest}"
    );
}

#[test]
fn render_replaces_the_openrouter_key_from_its_environment() {
    let tmpl = template("render-openrouter-tmpl");
    write(
        &tmpl.join("models.json.tmpl"),
        r#"{"baseUrl":"http://__BPPC_HOST__","apiKey":"__OMLX_KEY__","openrouter":"__OPENROUTER_KEY__"}"#,
    );
    let agent_dir = fresh_dir("render-openrouter-agent");
    let project = fresh_dir("render-openrouter-project");
    write(&project.join("CLAUDE.md"), "project\n");
    let out = run_prepare(
        "render-openrouter",
        &tmpl,
    ...
    let models: Value = serde_json::from_str(
        &std::fs::read_to_string(agent_dir.join("models.json")).expect("models"),
    )
    .expect("rendered models JSON");
    assert_eq!(models["openrouter"].as_str(), Some("router-key"));
}

#[test]
fn render_json_escapes_a_key_containing_a_double_quote() {
    let tmpl = template("render-quote-tmpl");
    let agent_dir = fresh_dir("render-quote-agent");
    let project = fresh_dir("render-quote-project");
    write(&project.join("CLAUDE.md"), "project\n");
    let out = run_prepare(
        "render-quote",
        &tmpl,
        ...
        &project,
        &["--json"],
        &[("OMLX_API_KEY", "key-with-\"-quote")],
    );
    assert_eq!(out.status.code(), Some(0), "stderr: {}", stderr(&out));
    let models: Value = serde_json::from_str(
        &std::fs::read_to_string(agent_dir.join("models.json")).expect("models"),
    )
    .expect("a quoted key still produces valid JSON");
    assert_eq!(models["apiKey"].as_str(), Some("key-with-\"-quote"));
}

#[test]
fn unknown_render_placeholder_is_a_warn_and_keeps_the_exit_successful() {
    let tmpl = template("render-unknown-tmpl");
    write(
        &tmpl.join("models.json.tmpl"),
        r#"{"apiKey":"__OMLX_KEY__","future":"__UNKNOWN_SETTING__"}"#,
    );
    let agent_dir = fresh_dir("render-unknown-agent");
    let project = fresh_dir("render-unknown-project");
    write(&project.join("CLAUDE.md"), "project\n");
    let out = run_prepare(
        "render-unknown",
        &tmpl,
    ...
    assert_eq!(
        out.status.code(),
        Some(0),
        "WARN does not change exit success: {}",
        stderr(&out)
    );
    let report_json = json_out(&out);
    let report = report_json["steps"]
        .as_array()
        .expect("steps")
        .iter()
        .find(|step| step["name"] == "render")
        .expect("render step");
    assert_eq!(report["status"].as_str(), Some("WARN"));
    assert!(report["detail"]
        .as_str()
        .expect("detail")
        .contains("__UNKNOWN_SETTING__"));
}

#[test]
fn invalid_rendered_json_fails_before_models_json_is_written() {
    let tmpl = template("render-invalid-tmpl");
    write(
        &tmpl.join("models.json.tmpl"),
        r#"{"apiKey":"__OMLX_KEY__""#,
    );
    let agent_dir = fresh_dir("render-invalid-agent");
    let project = fresh_dir("render-invalid-project");
    write(&project.join("CLAUDE.md"), "project\n");
    let out = run_prepare(
        "render-invalid",
        &tmpl,
    ...
    assert_eq!(
        out.status.code(),
        Some(6),
        "invalid JSON is a render failure: {}",
        stderr(&out)
    );
    let report_json = json_out(&out);
    let report = report_json["steps"]
...
#[test]
fn wiki_init_does_not_truncate_a_preexisting_file() {
    let tmpl = template("wiki-existing-tmpl");
    let agent_dir = fresh_dir("wiki-existing-agent");
    let project = fresh_dir("wiki-existing-project");
    write(&project.join("wiki/decisions.md"), "user decisions\n");
    let out = run_prepare(
        "wiki-existing",
        &tmpl,
        &agent_dir,
    ...
    assert_eq!(out.status.code(), Some(0), "stderr: {}", stderr(&out));
    assert_eq!(
        std::fs::read_to_string(project.join("wiki/decisions.md")).expect("decisions"),
        "user decisions\n"
    );
}
...
#[cfg(unix)]
#[test]
fn symlinked_wiki_directory_is_refused() {
    use std::os::unix::fs::symlink;

    let tmpl = template("wiki-symlink-tmpl");
    let agent_dir = fresh_dir("wiki-symlink-agent");
    let project = fresh_dir("wiki-symlink-project");
    let target = fresh_dir("wiki-symlink-target");
    symlink(&target, project.join("wiki")).expect("symlink wiki directory");
    let out = run_prepare(
        "wiki-symlink",
        &tmpl,
        &agent_dir,
    ...
    assert_eq!(
        out.status.code(),
        Some(6),
        "symlink refusal is a non-root failure: {}",
        stderr(&out)
    );
    let report_json = json_out(&out);
    let report = report_json["steps"]
    ...
    assert!(report["detail"]
        ...
        .contains("wiki path"));
    assert!(
        !target.join("index.md").exists(),
        "the symlink target stays untouched"
    );
}

#[test]
fn empty_home_falls_back_to_userprofile_for_the_key_settings() {
    let tmpl = template("home-empty-tmpl");
    let agent_dir = fresh_dir("home-empty-agent");
    let project = fresh_dir("home-empty-project");
    write(&project.join("CLAUDE.md"), "project\n");
    let userprofile = fresh_dir("home-empty-userprofile");
    write(
        &userprofile.join(".omlx/settings.json"),
        r#"{"auth":{"api_key":"userprofile-key"}}"#,
    );
    let out = Command::new(env!("CARGO_BIN_EXE_agent"))
        ...
        .arg(&project)
        .arg("--json")
        .current_dir(fresh_dir("home-empty-run"))
        .env_remove("HARNESS_DB")
        .env_remove("OMLX_API_KEY")
        .env_remove("OPENROUTER_API_KEY")
        .env("HOME", "")
    ...
    assert_eq!(out.status.code(), Some(0), "stderr: {}", stderr(&out));
    let models = std::fs::read_to_string(agent_dir.join("models.json")).expect("models");
    assert!(
        models.contains("userprofile-key"),
        "empty HOME must use USERPROFILE"
    );
}
...
fn exit_6_when_the_seed_fails_on_a_missing_template() {
    ...
    assert_eq!(
        out.status.code(),
        Some(6),
        "a non-root failure exits 6, stderr: {}",
        stderr(&out)
    );
    let v = json_out(&out);
    let steps = v["steps"].as_array().expect("steps");
    assert_eq!(
        steps.iter().find(|s| s["name"] == "seed").expect("seed")["status"].as_str(),
        Some("FAIL")
    );
    ...
    assert_eq!(
        steps.iter().find(|s| s["name"] == "root").expect("root")["status"].as_str(),
        Some("OK")
    );
}
...
fn missing_key_fails_render_with_the_var_name_and_exits_6() {
    ...
    assert!(
        !err.contains("happy-key"),
        "no key value may appear in any report"
    );
    assert!(
        !agent_dir.join("models.json").exists(),
        "a failed render writes nothing"
    );
}
...
fn key_falls_back_to_home_omlx_settings_json() {
    ...
    let models = std::fs::read_to_string(agent_dir.join("models.json")).expect("models.json");
    assert_eq!(
        models, r#"{"baseUrl": "http://127.0.0.1:8080/v1", "apiKey": "settings-key-1"}"#,
...
fn checkout_case_skips_seed_and_renders_in_place() {
    ...
    assert!(seed["detail"]
        .as_str()
        .expect("detail")
        .contains("used in place"));
    assert!(
        !tmpl.join(".terax-seed").exists(),
        "no stamp is written into a checkout"
    );
    assert!(
        std::fs::read_to_string(tmpl.join("models.json"))
            .expect("rendered in place")
...
fn stamp_and_version_flags_land_in_the_seed_stamp() {
    ...
    assert!(
        stamp.starts_with("1.2.3+") && stamp.len() == 22,
        "version-prefixed hash, got {stamp}"
    );
...
fn pi_usage_errors_exit_2() {
    ...
    assert!(
        stderr(&out).contains("usage: agent pi prepare"),
        "stderr: {}",
        stderr(&out)
    );
